...
void Money::addDollars(int dollars) {
	if (dollars > 0) {	//only add dollars if values are positive
		this->totalCents += dollars*100;
	}
}

void Money::addCents(int cents) {
	if (cents > 0) { //only add cents if cents are positive
		this->totalCents += cents;
	}
}

void Money::subtractDollars(int dollars) {
	if (dollars > 0) { //only subtract if values are positive
		this->totalCents -= dollars*100;
	}
}

void Money::subtractCents(int cents) {
	if (cents > 0) { //only subtract if values are positive
		this->totalCents -= cents;
	}
}
